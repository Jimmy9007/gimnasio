...
use Administracion\Formularios\RutinasForm;
use Administracion\Modelo\Entidades\Rutinas;
use Dompdf\Dompdf;

class RutinasController extends AbstractActionController {
    ...
    function getFormulario($action = '', $onsubmit = '', $idRutinas = 0) {
        ...
        if ($idRutinas != 0) {
            $rutinasOBJ = $this->getRutinasDAO()->getRutinas($idRutinas);
...
//        $filtro = "rutinas.estado = 'Eliminado'";
        return new ViewModel(array(
            'rutinass' => $this->getRutinasDAO()->getRutinass()
        ));
    }
...
//        $filtro = "medidas.estado = 'Eliminado'";
        return new ViewModel(array(
            'rutinass' => $this->getRutinasDAO()->getRutinasUsuario($idUsuario),
            'usuarioRutinas' => $this->getUsuarioRutinasDAO()->getRutinasUsuarioId($idUsuario),
        ));
    }
...
//            var_dump('Nombre de la persona seleccionada: ' . $usuarioUnicoMedidas[0]['medidasOBJ']->getNOM_USU());
            ...
            if ($form->isValid()) {
                ...
                $rutinasOBJ->setFechaRutina(date('Y-m-d H:i:s'));
                $this->getRutinasDAO()->guardar($rutinasOBJ);
    ...
    public function editAction() {
        $idRutinas = (int) $this->params()->fromQuery('idRutinas', 0);
        $action = 'edit';
        $onsubmit = 'return confirm("¿ DESEA GUARDAR ESTA RUTINA ?")';
        $form = $this->getFormulario($action, $onsubmit, $idRutinas);
        $request = $this->getRequest();
        if ($request->isPost()) {
            $form->setData($request->getPost());
            if ($form->isValid()) {
    ...
    public function getRutinasSeleccionadaAction() {
        $idRutinas = (int) $this->params()->fromQuery('idRutinas', 0);
        if (!$idRutinas) {
            return 0;
        }
        try {
            $rutinasOBJ = $this->getOpcionDAO()->getOpcion($idRutinas);
        } catch (\Exception $ex) {
            return 0;
        }
    ...
    public function anexarEjerciciosAction() {
        $idRutinas = (int) $this->params()->fromRoute('id1', 0);
        if ($idRutinas == 0) {
            return $this->redirect()->toRoute('administracion/default', array(
                        'controller' => 'rutinas',
                        'action' => 'index',
        ...
        $view = new ViewModel(array(
            'form' => $form,
            'ejercicios' => $this->getEjerciciosDAO()->getEjerciciosRutinas($idRutinas),
        ));
        return $view;
    }
    ...
    public function setEjerciciosRutinasAction() {
        $request = $this->getRequest();
        if ($request->isPost()) {
            $datos = $request->getPost()->toArray();
            $this->getRutinasDAO()->anexarEjercicio($datos);
            return $this->redirect()->toRoute('administracion/default', array(
                        'controller' => 'rutinas',
...
//        $valorLetras = new ValorEnLetras();
        $rutinasOBJ = $this->getRutinasDAO()->getRutinas($idRutina);
        $ejerciciosRutina = $this->getRutinasDAO()->getEjerciciosRutina($idRutina);
    ...
    public function deleteEjercicioAction() {
        ...
        if ($request->isPost()) {
            ...
            $resultado = $this->getRutinasDAO()->eliminarEjercicio($idEjercicio, $idRutina);
            $response->setContent(Json::encode(array('eliminado' => $resultado)));
        }
        return $response;
    }

    public function seleccionarRutinaAction() {
        $view = new ViewModel(array(
            'rutinas' => $this->getRutinasDAO()->getRutinass()
        ));
        $view->setTerminal(true);
        return $view;
    }

    public function getRutinaAction() {
        $idRutina = (int) $this->params()->fromQuery('idRutina', 0);
        if (!$idRutina) {
            return 0;
        }

        $form = $this->getFormulario('buscar', '', $idRutina);
        $view = new ViewModel(array(
            'form' => $form
        ));
        $view->setTemplate('administracion/rutinas/formularioRutina');
        $view->setTerminal(true);
        return $view;
    }
