...
function editFotoperfil(idPerfil) {
    $.get('/gimnasio/administracion/perfil/editfoto', {idPerfil: idPerfil}, setFormulario);
    bloqueoAjax();
}
function verCambiarcontrasena() {
    $.get('/gimnasio/administracion/usuario/cambiarcontrasena', {}, setFormulario);
}
...
function guardarNuevoPassword() {
    if (confirm("PARA QUE EL CAMBIO DE CONTRASEÑA SEA REGISTRADO LA SESION ACTUAL DEBE CERRARSE \n ¿ DESEA REGISTRAR EL CAMBIO DE CONTRASEÑA ?")) {
        $.post('/gimnasio/administracion/usuario/cambiarcontrasena', $("#formCambiarcontrasena").serialize(), setNuevoPassword, 'json');
        bloqueoAjax();
    }
    return false;
}
...
function setNuevoPassword(respuesta) {
    switch (parseInt(respuesta['error'])) {
        case 0:
            ...
            location.href = '/gimnasio/login/login/cerrarSesion';
            break;
        case 1:
